...
// STEP # 1 THEME SOUND ON LOAD! 
$(document).ready(function() {
	//Start the background sound
	// startSound('background', true);
	var theme = new Audio("sound/gameBegin.mp3");
	theme.volume = 1; 
	theme.play(); 

//STEP # 2: QUESTION BANK: create a bank of questions 5 for easy level, 5 for difficult level and 5 for the tougest level. 
//to get the data directly from open datbase (5 easy questions random)
...
//STEP # 3: ONCLICK FUNCTION ON CHOICES 

	$('.options').click(function(){
		a = $(this);
		console.log(a); //just to check is this "this" is working accordingly

		//if player chooses correct answer 
		$('.options').addClass('disabled');
		if(a  == answer){
		// if(a.find('span').html() == answer){
			//Play Right sound 
			startSound('rightsound', false);
			a.addClass('disabled correct');
			// a.fadeToggle().fadeToggle().addClass('disabled correct');
			level++
			raiseLevel();
			setTimeout(function() {
				nextQuestion();
			...
			a.addClass('disabled wrong');
			a.fadeToggle().fadeToggle().addClass('disabled wrong');
